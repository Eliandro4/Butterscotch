...
static LibretroSoundInstance* findFreeSlot(LibretroAudioSystem* ma) {
    repeat(LIBRETRO_MAX_SOUND_INSTANCES, i) {
        if (!ma->instances[i].active) {
            return &ma->instances[i];
        }
    }

    LibretroSoundInstance* best = nullptr;
    repeat(LIBRETRO_MAX_SOUND_INSTANCES, i) {
        LibretroSoundInstance* inst = &ma->instances[i];
...
static LibretroSoundInstance* findInstanceById(LibretroAudioSystem* ma, int32_t instanceId) {
    int32_t slotIndex = instanceId - LIBRETRO_SOUND_INSTANCE_ID_BASE;
    if (0 > slotIndex || slotIndex >= LIBRETRO_MAX_SOUND_INSTANCES) return nullptr;
    LibretroSoundInstance* inst = &ma->instances[slotIndex];
    if (!inst->active || inst->instanceId != instanceId) return nullptr;
    return inst;
}

static char* resolveExternalPath(LibretroAudioSystem* ma, Sound* sound) {
...
static void libretroDestroy(AudioSystem* audio) {
    LibretroAudioSystem* ma = (LibretroAudioSystem*) audio;
    ...
    if (ma->engineReady) {
        ...
        repeat(LIBRETRO_MAX_AUDIO_STREAMS, i) {
...
static void libretroUpdate(AudioSystem* audio, float deltaTime) {
    LibretroAudioSystem* ma = (LibretroAudioSystem*) audio;
...
static int32_t libretroPlaySound(AudioSystem* audio, int32_t soundIndex, int32_t priority, bool loop) {
    LibretroAudioSystem* ma = (LibretroAudioSystem*) audio;
    if (!ma->engineReady) return -1;

    bool isStream = (soundIndex >= LIBRETRO_AUDIO_STREAM_INDEX_BASE);
    Sound* sound = nullptr;
    char* streamPath = nullptr;

    if (isStream) {
        int32_t streamSlot = soundIndex - LIBRETRO_AUDIO_STREAM_INDEX_BASE;
        if (0 > streamSlot || streamSlot >= LIBRETRO_MAX_AUDIO_STREAMS || !ma->streams[streamSlot].active) {
    ...
    LibretroSoundInstance* slot = findFreeSlot(ma);
    if (slot == nullptr) {
    ...
    slot->instanceId = LIBRETRO_SOUND_INSTANCE_ID_BASE + slotIndex;
...
static void libretroStopSound(AudioSystem* audio, int32_t soundOrInstance) {
    LibretroAudioSystem* ma = (LibretroAudioSystem*) audio;
    if (!ma->engineReady) return;

    if (soundOrInstance >= LIBRETRO_SOUND_INSTANCE_ID_BASE) {
        LibretroSoundInstance* inst = findInstanceById(ma, soundOrInstance);
        if (inst != nullptr) {
...
static void libretroStopAll(AudioSystem* audio) {
    LibretroAudioSystem* ma = (LibretroAudioSystem*) audio;
    if (!ma->engineReady) return;
...
static bool libretroIsPlaying(AudioSystem* audio, int32_t soundOrInstance) {
    LibretroAudioSystem* ma = (LibretroAudioSystem*) audio;
...
static void libretroPauseSound(AudioSystem* audio, int32_t soundOrInstance) {
    LibretroAudioSystem* ma = (LibretroAudioSystem*) audio;
    if (!ma->engineReady) return;
...
static void libretroResumeSound(AudioSystem* audio, int32_t soundOrInstance) {
    LibretroAudioSystem* ma = (LibretroAudioSystem*) audio;
    if (!ma->engineReady) return;
...
static void libretroPauseAll(AudioSystem* audio) {
    LibretroAudioSystem* ma = (LibretroAudioSystem*) audio;
    if (!ma->engineReady) return;
    repeat(LIBRETRO_MAX_SOUND_INSTANCES, i) {
        LibretroSoundInstance* inst = &ma->instances[i];
...
static void libretroResumeAll(AudioSystem* audio) {
    LibretroAudioSystem* ma = (LibretroAudioSystem*) audio;
    if (!ma->engineReady) return;
    repeat(LIBRETRO_MAX_SOUND_INSTANCES, i) {
        LibretroSoundInstance* inst = &ma->instances[i];
...
static void libretroSuspend(AudioSystem* audio) {
    (void)audio;
}

static void libretroResume(AudioSystem* audio) {
    (void)audio;
}

static void libretroSetSoundGain(AudioSystem* audio, int32_t soundOrInstance, float gain, uint32_t timeMs) {
    LibretroAudioSystem* ma = (LibretroAudioSystem*) audio;
    if (!ma->engineReady) return;
...
static float libretroGetSoundGain(AudioSystem* audio, int32_t soundOrInstance) {
    LibretroAudioSystem* ma = (LibretroAudioSystem*) audio;
...
static void libretroSetSoundPitch(AudioSystem* audio, int32_t soundOrInstance, float pitch) {
    LibretroAudioSystem* ma = (LibretroAudioSystem*) audio;
    if (!ma->engineReady) return;
...
static float libretroGetSoundPitch(AudioSystem* audio, int32_t soundOrInstance) {
    LibretroAudioSystem* ma = (LibretroAudioSystem*) audio;
...
static float libretroGetTrackPosition(AudioSystem* audio, int32_t soundOrInstance) {
    LibretroAudioSystem* ma = (LibretroAudioSystem*) audio;
    if (!ma->engineReady) return 0.0f;
...
static void libretroSetTrackPosition(AudioSystem* audio, int32_t soundOrInstance, float positionSeconds) {
    LibretroAudioSystem* ma = (LibretroAudioSystem*) audio;
    if (!ma->engineReady) return;
...
static float libretroGetSoundLength(AudioSystem* audio, int32_t soundOrInstance) {
    LibretroAudioSystem* ma = (LibretroAudioSystem*) audio;
    if (!ma->engineReady) return 0.0f;

    LibretroSoundInstance* match = nullptr;
    if (soundOrInstance >= LIBRETRO_SOUND_INSTANCE_ID_BASE) {
    ...
    if (soundOrInstance >= LIBRETRO_SOUND_INSTANCE_ID_BASE || soundOrInstance >= LIBRETRO_AUDIO_STREAM_INDEX_BASE)
        return 0.0f;
...
static void libretroSetMasterGain(AudioSystem* audio, float gain) {
    LibretroAudioSystem* ma = (LibretroAudioSystem*) audio;
    if (!ma->engineReady) return;
...
static void libretroGroupLoad(AudioSystem* audio, int32_t groupIndex) {
    if (groupIndex > 0 && audio->dw->agrp.count > (uint32_t) groupIndex) {
        ...
        FileSystem* fileSystem = ((LibretroAudioSystem*)audio)->fileSystem;
        ...
        DataWin *audioGroup = DataWin_parse(((LibretroAudioSystem*)audio)->fileSystem->vtable->resolvePath(((LibretroAudioSystem*)audio)->fileSystem, buf), options);
        arrput(audio->audioGroups, audioGroup);
    }
}

static bool libretroGroupIsLoaded(AudioSystem* audio, int32_t groupIndex) {
    return (arrlen(audio->audioGroups) > groupIndex);
}

static int32_t libretroCreateStream(AudioSystem* audio, const char* filename) {
    LibretroAudioSystem* ma = (LibretroAudioSystem*) audio;
    if (!ma->engineReady) return -1;
    ...
    int32_t streamIndex = LIBRETRO_AUDIO_STREAM_INDEX_BASE + freeSlot;
    fprintf(stderr, "Audio: Created stream %d for '%s' -> '%s'\n", streamIndex, filename, resolved);
    return streamIndex;
}

static bool libretroDestroyStream(AudioSystem* audio, int32_t streamIndex) {
    LibretroAudioSystem* ma = (LibretroAudioSystem*) audio;
    if (!ma->engineReady) return false;

    int32_t slotIndex = streamIndex - LIBRETRO_AUDIO_STREAM_INDEX_BASE;
    if (0 > slotIndex || slotIndex >= LIBRETRO_MAX_AUDIO_STREAMS) {
        fprintf(stderr, "Audio: Invalid stream index %d for destroy\n", streamIndex);
        return false;
    }

    LibretroAudioStreamEntry* entry = &ma->streams[slotIndex];
...
static AudioSystemVtable libretroAudioSystemVtable;

LibretroAudioSystem* LibretroAudioSystem_create(DataWin* dataWin, int32_t sampleRate) {
    LibretroAudioSystem* ma = safeCalloc(1, sizeof(LibretroAudioSystem));
    libretroAudioSystemVtable.init = libretroInit;
    libretroAudioSystemVtable.destroy = libretroDestroy;
    libretroAudioSystemVtable.update = libretroUpdate;
    libretroAudioSystemVtable.playSound = libretroPlaySound;
    libretroAudioSystemVtable.stopSound = libretroStopSound;
    libretroAudioSystemVtable.stopAll = libretroStopAll;
    libretroAudioSystemVtable.isPlaying = libretroIsPlaying;
    libretroAudioSystemVtable.pauseSound = libretroPauseSound;
    libretroAudioSystemVtable.resumeSound = libretroResumeSound;
    libretroAudioSystemVtable.pauseAll = libretroPauseAll;
    libretroAudioSystemVtable.resumeAll = libretroResumeAll;
    libretroAudioSystemVtable.suspend = libretroSuspend;
    libretroAudioSystemVtable.resume = libretroResume;
    libretroAudioSystemVtable.setSoundGain = libretroSetSoundGain;
    libretroAudioSystemVtable.getSoundGain = libretroGetSoundGain;
    libretroAudioSystemVtable.setSoundPitch = libretroSetSoundPitch;
    libretroAudioSystemVtable.getSoundPitch = libretroGetSoundPitch;
    libretroAudioSystemVtable.getTrackPosition = libretroGetTrackPosition;
    ...
    libretroAudioSystemVtable.createStream = libretroCreateStream;
    libretroAudioSystemVtable.destroyStream = libretroDestroyStream;
    ma->base.dw = dataWin;
    ma->base.vtable = &libretroAudioSystemVtable;
    ma->sampleRate = sampleRate > 0 ? sampleRate : 44100;
    return ma;
}
